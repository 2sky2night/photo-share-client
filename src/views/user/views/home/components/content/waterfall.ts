...
class WaterFallResolve {
  ...
  constructor(
    ...
    this.list = list;
    this.column = column;
    this.columnsHeight = Object.seal(
      Array.from<number>({ length: column }).fill(0)
    );
  ...
  updateColumn(num: number) {
    ...
    // 重置所有列数组
    this.columnsHeight = Object.seal(
      Array.from<number>({ length: this.column }).fill(0)
    );
    // 重新计算每项的空间
    this.columnWidth =
      (this.rootDOM.clientWidth - this.gap * (this.column - 1)) / this.column;
    // 重新计算每项的位置
  ...
  resolve() {
    ...
    out: for (let index = 0; index < this.childDOM.length; ) {
      ...
      // 从小到大排序当前所有列高度
      const columnList = this.columnsHeight
        .map((column, index) => ({ column: index, height: column }))
      ...
      for (let j = 0; j < columnList.length; j++) {
        ...
        if (target) {
          ...
          // 设置图片高度
          const imgContainer = target.querySelector(
            ".img-container"
          ) as HTMLElement;
          if (imgContainer) {
            imgContainer.style.height = this.getHeight(
              photo.photos[0].width,
              photo.photos[0].height
            );
          }
          // 设置图片项的宽度
          ...
          // 设置当前列的left偏移量
          target.style.left = `${
            (this.columnWidth + this.gap) * item.column
          }px`;
          // 设置当前列的height偏移量
