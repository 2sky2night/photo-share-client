<template>
  <div class="post-comment">
    <n-input type="textarea" v-model:value="content" maxlength="255" :resizable="false"
      :placeholder="$t('pleaseEnter', { title: $t('comment') })"></n-input>
    <div class="actions mt-10">
      <n-button @click="onHandleReset">{{ $t('reset') }}</n-button>
      <auth-action>
        <n-button @click="onHandlePost" :loading="isLoading" class="ml-5" type='primary'>{{ $t('send') }}</n-button>
      </auth-action>
    </div>
  </div>
</template>
