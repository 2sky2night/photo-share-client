<template>
  <div class="comment-list">
    ...
    <template v-if="pagination.firstLoading">
      ...
      <comment-list-skeleton :length="pagination.pageSize"></comment-list-skeleton>
    </template>
    <template v-else>
      <div class="header">
        <div class="text">
          <span class="title">{{ $t('comment') }}</span>
          <span class="ml-5 total">({{ $t('total') }} {{ pagination.total }} {{  }})</span>
        </div>
        <n-switch :loading="pagination.firstLoading || pagination.isLoading" v-model:value="pagination.desc"
      ...
      <template v-if="pagination.isLoading">
        ...
        <div class="pagination">
          <n-pagination :item-count="pagination.total" :page="pagination.page" :page-size="pagination.pageSize" />
        </div>
      </template>
      <template v-else>
        <!--有评论-->
        <template v-if="list.length">
          <div class="list-container">
            <comment v-for="item in list" :comment="item" :key="item.cid" v-model:is-liked="item.is_liked"
              v-model:like-count="item.like_count"></comment>
          </div>
          <div class="pagination">
            <n-pagination :item-count="pagination.total" @update:page="onHandlePageChange" :page="pagination.page"
...
const onHandleGetData = async () => {
  ...
  list.length = 0
  const res = await getCommentAPI(props.pid, pagination.page, pagination.pageSize, pagination.desc)
