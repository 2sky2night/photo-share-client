    ...
    <div class="form-container">
      ...
      <div class="main">
        ...
        <n-form :show-require-mark="false" :rules="rules" class="mt-10" :model="formData" ref="formIns">
          ...
          <n-form-item class="mt-10" :label="$t('rePassword')" path="rePassword">
            <n-input :placeholder="$t('pleaseEnter', { title: $t('password') })" v-model:value="formData.rePassword"
              type="password"></n-input>
          </n-form-item>
          <div class="action">
            <n-button class="mr-10" @click="onHandleReset">{{ $t('reset') }}</n-button>
            <n-button type="primary" @click="onHandleRegister">{{ $t('register') }}</n-button>
          </div>
        </n-form>
      </div>
    </div>
...
const rules: FormRules = {
  ...
  rePassword: [
    ...
    { min: 6, max: 14, trigger: ['blur', 'input'], message: i18n.global.t('fieldLengthError', { max: 14, min: 6, title: i18n.global.t('password') }) },
    {
      trigger: ['blur'], validator(_role, value: string) {
...
const onHandleRegister = async () => {
  if (formIns.value) {
    ...
    await reigterAPI({ username: formData.username, password: formData.password })
    message.success(i18n.global.t('success', { title: i18n.global.t('register') }))
    router.push('/login')
  }
}
